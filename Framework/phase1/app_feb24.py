...
@st.cache_resource
def load_lsnet():
    model = LSNet(embedding_dim=128).to(DEVICE)
    model.eval()
    return model
...
def extract_sample(npz_file, samples_by_type, drone_type=None):
    ...
    try:
        ...
        if sample['spectrogram'] is None:
            ...
            # Force normalization safety (in case old files exist)
            spec = sample['spectrogram']
            if spec.max() > 1.5 or spec.min() < -0.1:
                spec = 20 * np.log10(np.abs(spec) + 1e-6)
                spec = np.clip(spec, -80, 0)
                spec = (spec + 80) / 80
            sample['spectrogram'] = spec.astype(np.float32)
...
class KyberModule:
    def key_exchange(self):
        time.sleep(random.uniform(6, 10) / 1000)
        return {"session_key": "kyber_shared_secret"}

class DilithiumModule:
    def sign(self, payload):
        time.sleep(random.uniform(3, 6) / 1000)
        return {"signature": "dilithium_sig"}

    def verify(self, payload, drone):
        """
        Simulated Dilithium.Verify()
        """

        # Honest drones almost always valid
        if not drone.is_malicious:
            return random.random() > 0.02  # 98% pass

        # Malicious drones fail more often
        return random.random() > 0.6  # 40% pass
    def sign(self, payload):
        time.sleep(random.uniform(3, 6) / 1000)
        return {"signature": "dilithium_sig"}
...
class HybridPQCModule:
    ...
    def encrypt_update(self, update_payload, mode="HYBRID_PQC"):
        start = time.time()
        drone_id = update_payload.get("drone_id", "UNK")

        # Base FL payload size
        B_FL = len(update_payload.get("weights", [])) * 8 / 1024  # KB
        ...
        # ----------------------------
            ...
            self.kyber.key_exchange()
            self.dilithium.sign(update_payload)

            aes_time = random.uniform(0.2, 0.6)
            time.sleep(aes_time / 1000)

            L_crypto = random.uniform(10, 15)
...
class ByzantineResistantTrustManager:
    ...
    def calculate_sei_similarity(self, sei_params1, sei_params2):
        ...
        if not isinstance(sei_params1, dict) or not isinstance(sei_params2, dict):
            return 0.0
    ...
    def update_trust_score(self, drone_id, verification_result, sei_similarity, 
        ...
        current_score = self.trust_scores.get(drone_id, 0.5)
    ...
    def get_trust_level(self, drone_id):
        ...
        score = self.trust_scores.get(drone_id, 0.5)  # fallback for late init
        if score >= 0.8:
            return "TRUSTED"
        elif score >= 0.6:
            return "SUSPICIOUS"
...
class AdvancedDrone:
    ...
    def __init__(self, drone_id, drone_type, sample_data, is_trusted=True, is_malicious=False):
        ...
        # Manufacturer (MUST exist for all drones)
        if isinstance(sample_data, dict):
            self.manufacturer = str(sample_data.get("manufacturer", "UNKNOWN")).upper()
        else:
        ...
        if is_trusted:
            self.trust_score = random.uniform(0.75, 0.85)
        elif is_malicious:
            self.trust_score = random.uniform(0.20, 0.35)
        else:
            # UNKNOWN / OPEN-SET
            self.trust_score = random.uniform(0.25, 0.45)

        # Sync initial trust into Trust Manager
        if 'defense' in st.session_state and st.session_state.defense:
            st.session_state.defense.trust_manager.trust_scores[self.id] = self.trust_score
...
class DefenseArchitecture:
    ...
    def initialize_attack_patterns(self):
        ...
        return {
            'MODEL_POISONING': {
                'weight_deviation_threshold': 2.0,
                'gradient_magnitude_threshold': 1.5,
                'update_frequency_threshold': 1.2
            },
            'SYBIL': {
            ...
            'EVASIVE': {
                ...
                'communication_pattern_threshfold': 0.6
            }
        }
    ...
    def detect_model_poisoning(self, update, drone):
        ...
        return detection_score > 0.35, detection_score, reasons
    ...
    def zero_trust_handshake(self, drone, update):
        """
        Algorithm 2: Zero-Trust Swarm Entry
        Physical Challenge + Dilithium + Kyber
        """

        # ----------------------------------------
        # PHASE 1 — PHYSICAL CHALLENGE (Gate 1)
        # ----------------------------------------
        ...
        # Send nonce to UAV
        drone.challenge_nonce = nonce
        add_log(f"[ZTS] Nonce sent to {drone.label} | nonce={nonce}", "SECURITY")
        
        # Receive response r(t) = φ(e(t)) + n(t)

        iq_signal = drone.iq_data

        # v_sei ← LSNet(STFT(r(t)))
        _, _, Sxx = signal.stft(
            iq_signal,
            fs=FS,
            nperseg=N_PER_SEG,
        ...
        spec_tensor = torch.tensor(spec).unsqueeze(0).repeat(1, 2, 1, 1).float().to(DEVICE)

        with torch.no_grad():
            v_sei = st.session_state.lsnet(spec_tensor)

        v_sei = v_sei.cpu().numpy().flatten()

        # Step 8: δ ← CalculateDistance(v_sei, D_trust)

        stored = self.trust_manager.sei_fingerprints.get(drone.id)

        if stored:
            reference = stored["sei_params"]
            sei_similarity = self.trust_manager.calculate_sei_similarity(
                drone.sei_params, reference
            )
            delta = 1 - sei_similarity
        else:
            delta = 0.0

        # if δ > τ_adaptive then REJECT
        tau_adaptive = 1 - SEI_MATCH_THRESHOLD

        if delta > tau_adaptive:
            return False, None

        # ----------------------------------------
        # PHASE 2 — PQC AUTHENTICATION (Dilithium)
        # ----------------------------------------

        # Receive digital signature σ
        signature = self.pqc.dilithium.sign(update)

        # Verify signature
        signature_valid = self.pqc.dilithium.verify(update, drone)

        if not signature_valid:
            add_log(f"[ML-DSA] Signature verification FAILED: {drone.label}", "SECURITY")
            return False, None
        ...
        shared_secret = encapsulated["session_key"]

        #K_sess ← KDF(ss, nonce)
        session_key = hashlib.sha256(
            f"{shared_secret}_{nonce}".encode()
        ).hexdigest()
    ...
    def comprehensive_security_check(self, drone, update=None):
            ...
            if not hasattr(drone, "manufacturer"):
                if hasattr(drone, "sample_data") and isinstance(drone.sample_data, dict):
                    drone.manufacturer = str(
                        drone.sample_data.get("manufacturer", "UNKNOWN")
                    ).upper()
                else:
                    drone.manufacturer = "UNKNOWN"

            if not hasattr(drone, "sei_params") or not isinstance(drone.sei_params, dict):
                drone.sei_params = {}
...
def initialize_session_state():
    ...
    if "lsnet" not in st.session_state:
        st.session_state.lsnet = load_lsnet()
    ...
    if 'initialized' not in st.session_state:
        ...
        for drone in st.session_state.drones:
            ...
            # Manufacturer backfill (open-set safe)
            if not hasattr(drone, "manufacturer"):
                if hasattr(drone, "sample_data") and isinstance(drone.sample_data, dict):
                    drone.manufacturer = str(
                        drone.sample_data.get("manufacturer", "UNKNOWN")
            ...
            # Trust backfill (no KeyError ever again)
            if drone.id not in st.session_state.defense.trust_manager.trust_scores:
                st.session_state.defense.trust_manager.trust_scores[drone.id] = drone.trust_score
...
def run_fl_round():
    ...
    for drone in st.session_state.drones:
        ...
        # added on 24 feb 2024 - store report for first evaluation
        ...
        if trust > MIN_TRUST_SCORE:
            ...
            # added on 24 feb 2024 - store report for second evaluation
            st.session_state.security_reports.append(security_report)
...
def global_model_update(updates, drone_ids, beta=0.5):
    ...
    # Store dynamic threshold history
    if "delta_history" not in st.session_state:
        st.session_state.delta_history = []

    st.session_state.delta_history.append({
        "round": st.session_state.update_count,
...
def plot_sei_analysis(drone):
    ...
    if hasattr(drone, 'spectrogram_data') and drone.spectrogram_data is not None:
        ...
        if spec.ndim == 2:
            im = axes[3].imshow(spec[:256, :256], aspect='auto', origin='lower', cmap='hot')
...
def plot_pqc_comparison():
    ...
    modes = CRYPTO_MODES
    latencies = [
        np.mean([m["L_total"] for m in st.session_state.defense.phase2_metrics[mode]])
        for mode in modes
    ]

    ax.bar(modes, latencies)
...
# Dynamic threshold plot for FL
def plot_dynamic_threshold():
    if "delta_history" not in st.session_state or not st.session_state.delta_history:
        return None
...
def main():
    ...
    with col_left:
        ...
        mgmt_col1, mgmt_col2, mgmt_col3, mgmt_col4, mgmt_col5 = st.columns(5)
        ...
        # 🛰️ Add UNKNOWN UAV (NEW BUTTON)
        with mgmt_col3:
            if len(st.session_state.drones) < MAX_DRONES:
                if st.button("🛰️ Add Unknown", key="add_unknown",
                            use_container_width=True):
                    
                    unknown_types = [t for t in st.session_state.dataset_samples.keys() 
                                if 'UNKNOWN' in t]

                    if unknown_types:
                        drone_type = random.choice(unknown_types)
                        sample = random.choice(st.session_state.dataset_samples[drone_type])
                    else:
                        # Fallback: generate synthetic unknown
                        sei_params = generate_sei_params("UNKNOWN_NOISE")
                        snr_db = random.uniform(5, 15)

                        sample = {
                            'file_path': "synthetic_unknown.npz",
                            'drone_type': "UNKNOWN_NOISE",
                            'iq_data': generate_iq_from_sei(sei_params, snr_db),
                            'spectrogram': None,
                            'snr_db': snr_db,
                    ...
                    st.session_state.drone_counter += 1
                    new_drone = AdvancedDrone(
                        drone_id=f"U{st.session_state.drone_counter}",
                        drone_type=drone_type,
                        sample_data=sample,
                        is_trusted=False,      # Unknown ≠ trusted
                        is_malicious=False     # But not explicitly malicious
                    )

                    # Open-set behavior flags
                    new_drone.trusted = False
                    new_drone.is_malicious = False
                    new_drone.trust_score = 0.35   # Start below min trust
                    new_drone.sei_params = {}      # No stable fingerprint

                    st.session_state.drones.append(new_drone)
                    add_log(f"OPEN-SET UAV JOINED: {new_drone.label}", "WARNING")
    ...
    with col_right:
        ...
        st.markdown("### 🔐 PQC Mode Comparison")

        metrics = st.session_state.defense.phase2_metrics

        if all(len(metrics[m]) > 0 for m in CRYPTO_MODES):

            def avg(m, k):
                return np.mean([x[k] for x in metrics[m]])
            ...
            for i, mode in enumerate(CRYPTO_MODES):
                with [col1, col2, col3][i]:
                    st.markdown(f"**{mode}**")
                    st.metric("Avg Latency (ms)", f"{avg(mode, 'L_total'):.2f}")
                    st.metric("Crypto Delay (ms)", f"{avg(mode, 'L_crypto'):.2f}")
                    st.metric("Bandwidth (KB)", f"{avg(mode, 'B_total'):.2f}")
                    st.metric("Overhead (%)", f"{avg(mode, 'O_bw'):.1f}")
        else:
            st.info("Waiting for metrics from all crypto modes...")
